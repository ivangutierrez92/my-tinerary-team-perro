...
export default function MyEditHotels() {
  ...
  useEffect(() => {
   const getHotel= ()=>axios.get(`${process.env.REACT_APP_API_URL}/api/hotels/${hotelid}`)
  const getCities= ()=>axios.get(`${process.env.REACT_APP_API_URL}/api/cities`)
  
      Promise.all([getHotel(),getCities()])
      .then(([hotelRes,citiesRes]) => {
        setHotelEdit(hotelRes.data.response);
        setCity(citiesRes.data.response);
      })
      .catch((error) => {
  ...
  const onSubmit = (event) => {
    ...
    axios
      ...
      .then((response) => {
        swal("Hotel updated", "the Hotel was successfully updated", "success");
        nav(`/hotel/${response.data.id}`);
      })
      .catch((error) => {
        swal("Error updating Hotel", error.response.data.message, "error");
      });
  };
